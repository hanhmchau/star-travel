                    ...
                    <div class="mr-auto">
                        ...
                        <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                            <li class="m-nav__item m-nav__item--home">
                                <a href="/administrator" class="m-nav__link m-nav__link--icon">
                        ...
                        <div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
                            <div class="row align-items-center">
                                ...
                                <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                                    <a target="_blank" href="/signin" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                                        <span>
...
//                            template: '{{OrderID}} - {{ShipCountry}}'
                            ...
                            template: function(row, index, datatable) {
                                var status = {
                                    0: {'title': 'Banned', 'class': 'm-badge--danger'},
...
//                            overflow: 'visible',
                            template: function (row, index, datatable) {
                                ...
                                var command = row.status ? "Ban" : "Unban";
                                return '\
						<a target="_blank" href="'+contextPath+'/profile/settings/'+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">\
							<i class="la la-edit"></i>\
						</a>\
						<button data-status="'+row.status+'" data-value="'+row.id+'" type="button" class="btn-ban-user btn m-btn--pill m-btn--air btn-outline-danger m-btn ' +
                                    'm-btn--custom">'+ command +'</button>\
					            ';
                            }
                        }]
            ...
            $('body').on('click', '.btn-ban-user', function(e){
                var id = $(this).attr('data-value');
                $(this).closest('tr').addClass('to-delete');
                $.ajax({
                    url: '<s:url value="/administrator/SetUserStatusAction" />',
                    method: 'POST',
                    dataType: "json",
